...
<div class="container mx-auto p-6">
    <div class="max-w-4xl mx-auto">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold text-gray-800">🔍 إضافة فحص جودة جديد</h1>
        ...
        <div class="bg-white rounded-lg shadow-lg p-8">
            <form action="{{ route('quality-checks.store') }}" method="POST">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="production_order_id" id="production_order_id" required
                            ...
                            <option value="">اختر طلب الإنتاج</option>
                            @foreach($productionOrders as $order)
                            <option value="{{ $order->id }}" {{ old('production_order_id') == $order->id ? 'selected' : '' }}>
                                #{{ $order->id }} - {{ $order->product->name }} ({{ $order->quantity }} قطعة)
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="status" id="status" required
                            ...
                            <option value="passed" {{ old('status') == 'passed' ? 'selected' : '' }}>✅ تمت الموافقة</option>
                            <option value="failed" {{ old('status') == 'failed' ? 'selected' : '' }}>❌ فشل الفحص</option>
                        </select>
                        @error('status')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div class="mt-6">
                    ...
                    <div id="defects-container">
                        <div class="flex items-center space-x-2 mb-2">
                            <input type="text" name="defects[]" placeholder="نوع العيب"
