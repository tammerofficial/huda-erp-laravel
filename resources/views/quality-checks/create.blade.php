...
<div class="container mx-auto p-6">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-3xl font-bold">🔍 Add Quality Check</h1>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="product_id" id="product_id" required
                        ...
                        @foreach($productionOrders as $order)
                            @if($order->product)
                                <option value="{{ $order->product->id }}" {{ old('product_id') == $order->product->id ? 'selected' : '' }}>
                                    {{ $order->product->name }}
                                </option>
                            @endif
                        @endforeach
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="defects" id="defects" rows="3"
                              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                              placeholder='{"defect1": "description", "defect2": "description"}'></textarea>
                    @error('defects')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="notes" id="notes" rows="4"
                              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                              placeholder="Additional notes about the quality check..."></textarea>
                    @error('notes')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
