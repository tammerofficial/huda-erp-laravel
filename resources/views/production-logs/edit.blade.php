...
<div class="container mx-auto p-6">
    <div class="max-w-4xl mx-auto">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold text-gray-800">Edit Production Log #{{ $productionLog->id }}</h1>
            ...
            <form action="{{ route('production-logs.update', $productionLog) }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="employee_id" id="employee_id" required
                            ...
                            <option value="{{ $employee->id }}" {{ old('employee_id', $productionLog->employee_id) == $employee->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="production_stage_id" id="production_stage_id" required
                            ...
                            <option value="{{ $stage->id }}" {{ old('production_stage_id', $productionLog->production_stage_id) == $stage->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="end_time" id="end_time"
                               value="{{ old('end_time', $productionLog->end_time?->format('Y-m-d\TH:i')) }}"
                               class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
                        @error('end_time')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div class="mt-6">
                    ...
                    <textarea name="notes" id="notes" rows="4"
                              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                              placeholder="Enter any additional notes about the production...">{{ old('notes', $productionLog->notes) }}</textarea>
                    @error('notes')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
